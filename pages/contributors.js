...
function ContributorCard({ data }) {
  return (
    <>
      <div className="relative my-6 mx-3 h-[353px] w-[265px] rounded-xl dark:bg-white bg-[#2F3F50] border-white dark:border-black dark:shadow-xl border-[0.5px]">
        <img
          src={data.img}
          className="py-4 mx-3 h-[245px] w-[240px] rounded-2xl object-cover"
        />
        <div className="m-3 flex justify-between items-center">
          <div>
            <div className="text-2xl font-semibold">{data.name}</div>
            <div className="font-light opacity-50">{data.job}</div>
          </div>
          <div className="flex">
            <a href={data.github}>
              <svg
